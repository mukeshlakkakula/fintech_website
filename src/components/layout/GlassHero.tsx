      ...
      <div className="container mx-auto px-6 md:px-12">
        <div className="max-w-5xl mx-auto relative">
          <motion.div
            ...
            className="bg-background-secondary/30 dark:bg-white/5 backdrop-blur-3xl border border-foreground/10 rounded-[1.5rem] md:rounded-[4rem] p-4 md:p-24 shadow-2xl flex flex-col md:flex-row items-center gap-10 md:gap-16"
          >
            <div className="flex-1 text-center md:text-left">
              <nav className="flex items-center justify-center md:justify-start gap-2 text-brand text-xs md:text-sm font-bold uppercase tracking-widest mb-6 md:mb-8">
                <Link href="/" className="hover:text-brand transition-colors">Home</Link>
                <ChevronRight className="w-3 h-3 md:w-4 md:h-4" />
                <span className="text-foreground/60">{breadcrumb}</span>
              </nav>
              <h1 className="text-4xl md:text-6xl font-heading font-bold text-foreground mb-6 md:mb-8 leading-[1.1] md:leading-tight">
                {title}
              </h1>
              <p className="text-lg md:text-xl text-foreground/50 leading-relaxed max-w-2xl mx-auto md:mx-0">
                {subtitle}
              </p>
            </div>
